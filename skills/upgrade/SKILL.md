...
---
<!-- PhanesLightUpgrade v3.7.2, 2026-09-04, production upgrade prompt for existing PhanesLight installations.
...
| `.phaneslight/scripts/*` | **REGENERATE** | The target spec defines the current script set. From v3.4 that set is ten commands larger on Windows, and from v3.7.2 ten larger on POSIX too, so a POSIX project gains ten scripts on this upgrade where earlier ones correctly gained none. |
...
<!-- LEGACY-NAME-BLOCK BEGIN: the identifiers below are pre-v3.6.0 names, matched literally against what is on disk. Do NOT rename them. -->
   ...
   * **Reinstall the script library** from the v3.7.2 manifest into `.phaneslight/scripts/`, which is what brings in the renamed dispatcher and the new `phaneslight-template` stamps, then smoke-run it: `node .phaneslight/scripts/cli.js module-list`. Do **NOT** delete `.phanes/scripts/` yet.
...
4. **New-system health.** The three hook scripts present on disk in `.phaneslight/scripts/` under this platform's extension, and no pre-v3.7.0 Phanes hook entry left in `.claude/settings.json` (registration lives in the plugin's own `hooks/hooks.json` from v3.7.0, so a surviving project-level entry is stale, not required); `phaneslight doc-check` and `phaneslight loc-check` run clean or produce only known flags; registry tier 1 freshly generated; every `documentation/` folder (minus exemptions) carries an `_index.md`; `.phaneslight/manifest.json` parses and every listed path exists; `.phaneslight/config.json` carries `phanesLightVersion` = target.
...
15. **Bootstrap command set present and dispatching (target v3.4).** Platform-conditional, and the POSIX branch is a real pass rather than a skip. **On Windows:** all ten new commands are installed in `.phaneslight/scripts/` and dispatch through the cross-shell entry. Verify by running two of them rather than by listing files, since a file that exists and does not dispatch is the failure this item exists to catch: `node .phaneslight/scripts/cli.js preflight` emits JSON carrying `runType` and `legacyMarkers`, and `node .phaneslight/scripts/cli.js update-preflight` emits a verdict whose `gitDelta` object carries `worktreeDirty`. **On POSIX (v3.7.2):** the same two invocations **MUST SUCCEED**, emitting the same keys, with `preflight` reporting `"platform": "posix"`. A refusal by name is now the defect it used to be the correct answer to: the dispatcher's refusal block is gone as of v3.7.2, so a project still refusing these has a library installed before the port and the script regeneration item is what repairs it. Either platform's wrong answer reopens the script regeneration item.
16. **Session-start ledger hook script present (target v3.4; both platforms from v3.7.2).** `hook-ledger-status` is on disk in `.phaneslight/scripts/` under this platform's extension, `.ps1` on Windows and `.sh` on POSIX. **A missing script on a project installed before v3.4, or on a POSIX project installed before v3.7.2, is a repairable absence rather than a user deletion**, and this upgrade installs it; the two are distinguishable because the project's recorded `phanesLightVersion` predates the version that ships it. Registration is not this item's business: the plugin registers the `SessionStart` entry from its own `hooks/hooks.json` and the launcher resolves the platform extension, so there is nothing to look for in `.claude/settings.json`. Verify the hook body itself is silent on a healthy project: with a closed or absent `.phaneslight/run-progress`, running it prints nothing and exits 0. Silence is this hook's healthy signal, so a hook that speaks on a healthy project is as much a defect as one that stays quiet on a dead run.
...
24. **Legacy hook paths gone (target v3.6.0, rescoped at v3.7.0).** No Phanes hook command anywhere in `.claude/settings.json` still names `.phanes/scripts/`, and any surviving Phanes entry there is removed rather than repointed, because the plugin registers all three hooks from its own `hooks/hooks.json` and item 26 requires the project-level table to be empty of them. Every hook script named by that registration exists on disk under this platform's extension, and `node .phaneslight/scripts/cli.js module-list` exits 0. Confirm the user was told to restart the session; hook changes are inert until they do. **(v3.7.2)** This item and item 4 asserted until now that hook entries must be PRESENT in `.claude/settings.json`, which item 26 has contradicted since v3.7.0 and which no compliant project has satisfied since then.
<!-- LEGACY-NAME-BLOCK END -->
