...
---
<!-- PhanesLight v3.7.1 (2026-09-04). Bootstrap prompt, the /phaneslight:run skill of the PhanesLight plugin.
...
* **`phaneslight hook-verify`** folds into `update-preflight` as both a standalone invocation and a sensor: it reports the hook table's state (present, missing, mis-pathed) and repairs nothing. Repair is Step 4b's duty, and under the plugin that duty is putting a correctly named script back on disk in `.phaneslight/scripts/`; registration lives in the plugin's `hooks/hooks.json` and is never written into the user's `.claude/settings.json`.
