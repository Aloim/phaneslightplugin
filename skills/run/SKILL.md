...
**(v3.4, still true) `hook-ledger-status` is Windows only this cycle**, because it has no POSIX sibling until the port lands. **(v3.7.1)** The `SessionStart` entry can no longer be omitted per platform, since `hooks/hooks.json` ships to every platform identically, so the launcher carries the omission instead: a hook whose platform variant is not on disk is a **silent no-op, exit 0**, which is also what happens when the plugin is enabled in a project that has no `.phaneslight/` at all. Generate no POSIX `hook-ledger-status.sh` until the real port lands; a stub that prints nothing is indistinguishable from a healthy ledger, which is the one thing this hook exists to never say.
