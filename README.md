...
A listing in a community collection is **submitted, not live**: the pull request to `davepoon/buildwithclaude` is open and under review. Until it merges, the pair above is the only install path, and this README will name the collection install here once it exists rather than before.
